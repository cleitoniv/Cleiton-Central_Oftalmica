defmodule TecnovixWeb.CreditoFinanceiroController do
  ...
  alias Tecnovix.{CreditoFinanceiroModel, ClientesSchema, UsuariosClienteSchema, LogsClienteModel, NotificacoesClienteModel}
  ...
  def create(conn, %{"items" => params, "id_cartao" => id_cartao}) do
    ...
    with {:ok, items_order} <- CreditoFinanceiroModel.items_order(params),
         ...
         {:ok, _notifications} <- NotificacoesClienteModel.credit_finan_adquired(credito, cliente),
         {:ok, _logs} <-   LogsClienteModel.create(ip,
                              usuario,
                              cliente,
                              "#{credito.valor} Creditos Financeiros adicionado com sucesso.") do
        conn
        |> put_status(200)
