defmodule TecnovixWeb.Auth.SyncUsers do
  ...
  defp verify_token(token) do
    case Phoenix.Token.verify(TecnovixWeb.Endpoint, @sync_users_salt, token, max_age: 3_600 * 200) do
      {:ok, %{type: "token"}} ->
        Phoenix.Token.verify(TecnovixWeb.Endpoint, @sync_users_salt, token, max_age: 3_600)

      {:ok, %{type: "refresh_token"}} ->
        Phoenix.Token.verify(TecnovixWeb.Endpoint, @sync_users_salt, token, max_age: 3_600 * 12)
